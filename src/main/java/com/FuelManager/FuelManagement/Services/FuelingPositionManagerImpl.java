...
public class FuelingPositionManagerImpl implements FuelingPositionManager {
    ...
    public List<Integer> getActiveFuelingPositionById() {
        return fuelingPositionRepo.findAll().stream().map(FuelingPosition::getFuelingPositionNumber).filter(x -> transactionManager.hasActiveTransaction(x)).toList();
    }

    @Override
    public List<Integer> getInactiveFuelingPositionById() {
        try {
            return fuelingPositionRepo.findAll().stream().filter(fp -> transactionBuilder.getTransaction().getEquipment().getProducts().stream().map(Product::getProductName).anyMatch(x -> x.equals(fp.getProduct().getProductName()))).map(FuelingPosition::getFuelingPositionNumber).filter(Predicate.not(x -> transactionManager.hasActiveTransaction(x))).toList();
        } catch (Exception e) {
            return List.of();
        }
    }
